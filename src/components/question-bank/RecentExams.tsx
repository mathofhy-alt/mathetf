...
export default function RecentExams({refresh,onRestore,onCount}:{refresh:number;onRestore:(item:any,fresh:boolean)=>void|Promise<void>;onCount:(n:number)=>void}){
 ...
 {busy&&<p role="status" className="mt-2 text-brand-700">시험지 정보를 불러오고 있습니다…</p>}{error&&<p role="alert">{error}</p>}{!items.length&&!error&&<p className="mt-2 text-slate-500">완성한 시험지는 여기에 표시됩니다.</p>}
