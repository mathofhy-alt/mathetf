...
async def main():
    ...
    # Looking at the original trace, Problem 15 is on Page 5.
    page = doc[4]
...
17. **[초엄격: 다항식 연산자 띄어쓰기 강제]** HWP 수식 편집기의 버그를 막기 위해, 사칙연산 및 등호(`+`, `-`, `=`, `le`, `ge`) 앞뒤로는 **무조건 공백(띄어쓰기)**을 넣으세요! (예: `x^4 + 4x^3 - 6x^2 = 0`)"""
    ...
    print("Sending prompt for Problem 15 to Gemini...")
    resp = await parser.model.generate_content_async(
        [Image.open(BytesIO(img_bytes)), prompt],
