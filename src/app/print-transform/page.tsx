...
export const metadata: Metadata = {
    title: '프린트와 비슷한 수학 기출 찾기 | 수학ETF',
    description: '학교에서 받은 수학 프린트를 올리고 문제를 잘라내면, 같은 유형의 기존 기출문항을 찾아 출제 화면에 담을 수 있습니다.',
    keywords: [
    ...
    openGraph: {
        title: '프린트와 비슷한 수학 기출 찾기 | 수학ETF',
        description: '학교 프린트를 올리고 문제를 잘라내면 같은 유형의 기존 기출문항을 찾아 출제 화면에 담을 수 있습니다.',
    ...
    return (
        <div className="min-h-screen bg-[#F2F3F0] text-[#294437] font-sans">
            ...
            <section className="sr-only">
                <h1>학교 프린트와 비슷한 기존 기출문항 찾기</h1>
                <p>
                    학교에서 받은 수학 프린트(PDF)를 올리고 원하는 문제를 드래그로 잘라내면,
                    AI가 문제의 단원과 유형을 인식해 전국 기출에서 같은 유형의 기존 기출문항을 찾아 드립니다.
                    마음에 드는 변형문제를 골라 담으면 출제 화면에서 조합한 시험지를 한글용 HML로 저장할 수 있습니다.
                    내신 대비로 학교 프린트를 한 번 더 연습하고 싶을 때, 같은 유형의 다른 기출문제로 실전 감각을 키워 보세요.
                </p>
            </section>
            <PrintTransformClient isLoggedIn={isLoggedIn} />
            {!isLoggedIn && (
                <section className="max-w-3xl mx-auto px-4 pb-10 -mt-4">
                    <h2 className="text-xl sm:text-2xl font-black break-keep text-[#294437]">
                    ...
                    {(questionCount > 0 || schoolCount > 0) && (
                        <div className="flex gap-3 mt-4">
                            ...
                            {schoolCount > 0 && (
                                <div className="bg-white border border-slate-200 rounded-xl px-5 py-3 shadow-sm">
                                    <p className="text-2xl font-black text-[#426D36]">{schoolCount.toLocaleString()}</p>
                    ...
                    <p className="text-sm text-slate-600 mt-3 break-keep">
                        ...
                        <Link href="/question-bank" className="text-[#426D36] font-bold hover:underline">시험지 출제</Link>,
                        학교와 시험범위만 정하고 싶다면{' '}
                        <Link href="/predict" className="text-[#426D36] font-bold hover:underline">예상문제 뽑기</Link>를 써 보세요.
                    </p>
                </section>
            )}
