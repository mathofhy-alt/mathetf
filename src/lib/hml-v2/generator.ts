...
export function generateHmlFromTemplate(
    ...
    // ═══════════════════════════════════════════════════════════
    // [2026-08-30] 30 유지. 한 번 67 로 올렸다가 단이 넘쳐서 되돌렸다.
    //
    // 67 은 "캡쳐 이미지를 단 폭에 맞춰 넣었을 때" 의 기하 계산이다:
    //   캡쳐 폭 1054px 고정 · 단 폭 31040 HWPUNIT(110mm) → 29.4 HWPUNIT/px
    //   한 줄 1978 HWPUNIT(7.0mm) → 1978/29.4 = 67px
    // 계산 자체는 맞지만 **대상이 틀렸다.** 아래 MODE A(이미지 모드)가 `if (false)` 로 꺼져 있어
    // 실제 출력은 캡쳐가 아니라 **원본 XML 을 텍스트로 렌더링**한다.
    // 텍스트 모드는 수식이 객체로 들어가고 선택지가 줄바꿈돼 캡쳐보다 세로로 더 크다.
    // 그래서 67 을 쓰면 "들어간다" 고 판단하고 실제로는 단이 넘친다(사용자 확인, 8/30).
    //
    // 30 은 근거가 적힌 값은 아니지만 실측상 넘치지 않는다(4개 회차 × 1·2·3문제 전부 0).
    // 제대로 고치려면 캡쳐 높이라는 대리값 대신 **content_xml 의 단락·수식 수를 세야** 한다.
