    ...
    def extract_math_problems(self, pdf_path: str) -> List[Dict]:
        ...
        for page_num in range(total_pages):
            ...
            try:
                ...
                업로드된 1장짜리 PDF 페이지에 있는 '모든 수학 문제'를 빠짐없이 추출하여 오직 JSON 리스트 형식으로만 응답하세요. (만약 이 페이지에 문제가 전혀 단 1개도 없다면 빈 배열 `[]`을 응답하세요)
                ...
                - 학생이 혼자서 완벽하게 이해할 수 있도록, **시중 출판 교재의 정답 및 해설지처럼 최대한 상세하고 논리적인 풀이 과정을 모두 적어주세요.** 
                - 대한민국 고등학교 1학년 수학(수학 상, 수학 하) 교육과정 수준에 완벽히 맞게 쓰되, 단순 요약이 아니라 단계별(Step-by-step)로 왜 이런 수식이 전개되는지 상세히 설명해야 합니다.
                - 고1 범위를 벗어나는 공식(예: 로피탈, 미적분학의 기본정리)은 절대 쓰지 마세요.
                
                [🚨 최고 우선순위 JSON 문법 규칙 🚨]
                - 이 페이지에 보이는 모든 번호의 문제는 무조건 1개도 누락 없이 100% 추출해야 합니다.
                - 응답은 반드시 `[` 로 시작하여 `]` 로 끝나는 순수 JSON 배열만 출력하세요. 인사말이나 잡담, 생각 과정은 절대 금지됩니다.
