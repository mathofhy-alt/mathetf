...
def _extract_json_objects(text):
    ...
    try:
        ...
        print("Success: json.loads strict=False")
    ...
    try:
        ...
        print("Success: ast.literal_eval")
        return [obj] if isinstance(obj, dict) else obj
    except Exception as e:
        print("Failed ast.literal_eval:", e)
        pass
            ...
            if text[i] == '{': count += 1
                ...
                if count == 0:
                    try:
                        ...
                        print("Success: Bracket matching")
                        break
                    except Exception as e: 
                        print("Failed Bracket matching item:", e)
                        pass
    return objects


async def test_prob_15():
...
8. 해설은 해라체(-다)를 사용하고 마지막에 "따라서 정답은 [최종답안]이다."를 포함하세요."""
    ...
    try:
        ...
        if extracted:
            print("Successfully extracted!", len(extracted))
        else:
            print("Failed to extract anything.")
            print("\nTo deep debug, saving raw_text to raw_prob15.txt")
            with open("raw_prob15.txt", "w", encoding="utf-8") as fOut:
                fOut.write(raw_text)
