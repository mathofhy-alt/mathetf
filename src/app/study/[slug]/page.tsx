    ...
    return (
        <div className="min-h-screen bg-[#F2F3F0] text-[#294437] font-sans">
            ...
            <main className="max-w-3xl mx-auto px-4 py-8 sm:py-10">
                {/* 히어로 */}
                <div className="inline-flex items-center gap-1.5 bg-[#88A96D]/12 border border-[#88A96D]/30 text-[#638747] text-xs font-bold px-3 py-1 rounded-full mb-3">
                ...
                <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-6 mt-6 space-y-3">
                    ...
                    <p className="text-xs text-slate-400 pt-1">
                        수학ETF가 보유한 <strong className="text-[#426D36]">{g.subject} 기출 {total.toLocaleString()}문항</strong>으로 예습 예상문제를 만들 수 있어요.
                    </p>
                </section>
                    ...
                    {g.chapters.map((c) => (
                        <div key={c.name} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                            <div className="flex items-center justify-between gap-2">
                                <h3 className="font-extrabold text-[#294437] break-keep">{c.name}</h3>
                                <span className="text-[11px] font-bold text-[#426D36] bg-[#EAF1E1] border border-[#C5D8B5]/60 px-2 py-0.5 rounded-full shrink-0">
                                    {chapterTotal(c)}문항
                                </span>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-1.5 mt-3">
                                {c.units.map((u) => (
                                    <span key={u} className="text-[11px] bg-slate-50 text-slate-600 border border-slate-200 px-2 py-0.5 rounded-full">
                                        {u} <span className="text-[#426D36] font-bold">{counts[u] || 0}</span>
                                    </span>
                                ))}
                            </div>
                        </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm divide-y divide-slate-100 overflow-hidden">
                    {g.roadmap.map((r) => (
                        <div key={r.weeks} className="flex gap-3 p-4">
                            <span className="text-xs font-extrabold text-white bg-[#426D36] rounded-lg px-2.5 py-1 h-fit whitespace-nowrap">{r.weeks}</span>
                ...
                {/* CTA (예상문제 뽑기 1순위) */}
                <div className="mt-8 bg-gradient-to-br from-[#426D36] to-[#638747] rounded-2xl p-6 text-center text-white shadow-md">
                    ...
                    <div className="flex flex-col sm:flex-row justify-center gap-2.5">
                        <Link href="/predict" className="inline-flex items-center justify-center gap-2 bg-white text-[#426D36] font-extrabold px-6 py-3 rounded-xl hover:bg-slate-50 transition-colors">
                ...
                <div className="mt-6 grid sm:grid-cols-3 gap-2">
                    {g.related.map((r) => (
                        <Link key={r.href} href={r.href} className="flex items-center justify-between gap-2 bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3 text-sm font-bold text-slate-700 hover:text-[#426D36] hover:shadow-md transition-all">
                            {r.label}<ChevronRight size={16} className="text-[#426D36]" />
                        </Link>
                    ))}
                </div>
            </main>
