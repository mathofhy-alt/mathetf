...
class GeminiMathParser:
    ...
    async def extract_math_problems(self, pdf_path: str, log_callback: Optional[Callable[[str], None]] = None, generate_variants: bool = False) -> List[Dict]:
        """
        [고속 분석 엔진 V2]
        ...
        if generate_variants and all_problems:
            ...
            for p in all_problems.copy():
                for v_idx in range(1, 4):
                    variant_tasks.append(self._generate_single_variant(p, v_idx, extract_semaphore, _log))
...
- 🚨 **[분수 및 괄호 수식 절대 규칙 - 필수]** 🚨
  ...
  연립방정식 등도 `\\\\begin{{cases}} ... \\\\end{{cases}}` 등 표준 LaTeX 문법만 사용하세요."""
    ...
    async def _generate_single_variant(self, original_obj: Dict, variant_idx: int, extract_semaphore, log_fn):
        ...
        async with extract_semaphore:
            ...
            while retries < 4:
                try:
                    if retries == 0:
                        log_fn(f"    [{var_q_num}] 병렬 생성 시작...")
                    else:
                        log_fn(f"    [{var_q_num}] 병렬 생성 재시도 ({retries}/4)...")
                        
                    prompt = f"""당신은 수학 문제 출제 및 변형 전문가입니다.
아래 제공된 '원본 수학 문제'와 그 해설을 완벽하게 분석하여, 핵심 원리와 출제 의도는 100% 동일하게 유지하되 숫자, 함수식, 혹은 상황을 살짝 바꾼 새로운 '변형 문제' 1개를 만들어주세요.
...
[새로 생성할 변형 문제 규칙]
1. 원본 문제와 난이도, 풀이 방식이 거의 똑같아야 합니다.
2. 변형 문제에 대한 해설도 원본 해설과 유사한 스텝으로 제공해야 합니다.
3. 출력은 반드시 아래 형태의 단일 JSON 객체를 포함하는 배열이어야 합니다.
[
  {{
...
🚨 [수식 포맷팅 절대 규칙]
모든 수학 수식, 변수, 숫자, 기호는 절대 일반 텍스트로 쓰지 말고 무조건 예외 없이 [[EQUATION:수식]] 특수 태그로 감싸야 합니다! 숫자가 나오는 아주 사소한 부분도 모두 감싸세요. 분수는 \\frac{{}}{{}} 형태를 쓰세요.
1차 방정식 등 단순한 변환을 주어 문제를 만들어도 좋습니다. 해설 텍스트 길이는 400자 이내로 핵심만 간결하게 설명하고 깊은 내부 추론(thought 등)을 피하세요."""
