...
class CropReviewDialog:
    ...
    def _build_ui(self):
        ...
        self.win.geometry("1400x1000")
        self.win.state("zoomed")  # 시작 시 전체화면(최대화)
        ...
        def _on_global_mousewheel(event):
            ...
            # 썸네일 영역에 마우스가 있으면 썸네일 캔버스 스크롤
            if widget == self._thumb_canvas or widget == self._thumb_frame or str(widget).startswith(str(self._thumb_frame)):
                self._thumb_canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")
            else:
                self._canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")

        def _on_global_shift_mousewheel(event):
            # Shift 스크롤은 수평 이동 (주 캔버스)
            self._canvas.xview_scroll(int(-1 * (event.delta / 120)), "units")

        self.win.bind("<MouseWheel>", _on_global_mousewheel)
        self.win.bind("<Shift-MouseWheel>", _on_global_shift_mousewheel)
    ...
    def _start_add_mode(self):
        ...
        self._is_drawing = False
        self._next_auto_num = None
        self._next_auto_prefix = None
        self._canvas.delete("box")
        self._draw_boxes()
        self._status_label.config(
    ...
    def _on_drag_end(self, event):
        ...
        # ── 새 문항 추가 모드 ────────────────────────────────────────────────
        if self._is_adding:
            # 연속 추가를 위해 self._is_adding = False 와 self._canvas.config 를 호출하지 않음
            if self._drag_rect_id:
                self._canvas.delete(self._drag_rect_id)
                self._drag_rect_id = None
            self._drag_start = None
            ...
            if getattr(self, '_next_auto_num', None) is None:
                # 문항 번호 입력 다이얼로그 (처음 한 번만)
                from tkinter.simpledialog import askstring
                q_num = askstring("문항 번호 입력",
                                  f"이 영역의 문항 번호를 입력하세요:\n(예: 1, 2, 3-1, ...)",
                                  parent=self.win)
                if not q_num:
                    self._status_label.config(text="추가 취소됨.")
                    self._is_adding = False
                ...
                if match:
                    ...
                    prefix = q_num[:-len(num_part)]
                    self._next_auto_prefix = prefix
                    self._next_auto_num = int(num_part) + 1
                else:
                    self._next_auto_prefix = q_num
                    self._next_auto_num = 1
            else:
                q_num = f"{self._next_auto_prefix}{self._next_auto_num}"
                self._next_auto_num += 1
            ...
            self._status_label.config(text=f"✅ {q_num}번 문항 추가됨! 연속해서 다음 영역을 그리거나 '확인'을 누르세요 (ESC: 종료).")
            return
