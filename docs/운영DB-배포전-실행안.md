...
배포 전 별도 작업본 점검: 변경 파일 TypeScript 검사, 개선 항목 검사 18개, Next 배포용 컴파일을 통과했습니다. 결제 완료 시 PC 직접 결제와 결제사 화면 복귀 모두 서버가 해당 주문에서 구매한 자료만 장바구니에서 제거하도록 수정했습니다. 2026-09-24 새벽, 별도 테스트 Supabase 프로젝트와 KG이니시스 테스트 채널에서 가상 PDF 1건(1,100원)을 실제 카드 승인까지 확인했습니다. 주문은 완료 상태이며 결제 기록 1건, 구매 기록 1건, 해당 장바구니 항목 0건입니다. 완료 주문의 복귀 주소를 다시 열어도 구매 완료로 표시됐고 기록 수가 늘지 않았습니다. 실제 휴대전화 결제 앱에서 브라우저로 돌아오는 과정은 별도로 확인하지 않았습니다.
...
한글 2014에서 실제 문항 5개로 만든 HML을 불러와 4쪽 PDF로 출력했습니다. 수식, 그림, 쪽 경계의 누락·잘림은 보이지 않았습니다. 이 확인은 5문항 표본에 한정됩니다.
