        ...
        <div className="min-h-screen bg-[#f3f4f6] text-slate-900 font-sans">
            ...
            <div className="bg-white border-b border-slate-200">
                <div className="max-w-[1200px] mx-auto px-4 h-16 flex items-center justify-between">
                    ...
                    {user && user.email === 'mathofhy@naver.com' && (
                        <Link href="/notice/write" className="flex items-center gap-2 px-4 py-2 bg-brand-600 text-white rounded text-sm font-bold hover:bg-brand-700">
                            <PenSquare size={16} />
                            <span>글쓰기</span>
                        </Link>
                    )}
                            ...
                            {loading ? (
                                ...
                                notices.map((notice, index) => (
                                    <tr key={notice.id} className="hover:bg-slate-50 transition-colors">
                                        <td className="px-6 py-4 text-center text-slate-400">{notices.length - index}</td>
                                        <td className="px-6 py-4">
                                            <Link href={`/notice/${notice.id}`} className="block font-medium text-slate-800 hover:text-brand-600">
                                                {notice.title}
                                            </Link>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-center text-slate-400">{notice.views}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
