...
export function generateHmlFromTemplate(
    ...
    try {
        ...
        if (mappingTable) {
            ...
            // [FIX ENDNOTE INVISIBLE] CharShape "14"(흰색/1pt)는 MAPPINGTABLE에 없어도 항상 허용
            validStyles.CharShape.add('14');
    ...
    // ═══════════════════════════════════════════════════════════
    // [2026-08-30] 30 → 45. 한글 COM 으로 실제 조판해서 잰 값이다(추정 아님).
    //
    // 측정 방법: 생성기로 HML 을 만들고 → 한글 COM(HWPFrame.HwpObject)으로 열어 PDF 저장 →
    //   PyMuPDF 로 문항별 시작 y ~ 내용 끝 y 를 재서 실제 높이(pt)를 얻었다.
    //   배명고 2025 2학기중간 수학II(18문항) + 양정고 2026 1학기중간 기하(19문항) = 37문항.
    //   (scratch_measure_gen.ts + scratch_hwp2pdf.py 로 재현 가능)
    //
    // 알아낸 것:
    //   - 생성 결과는 **1단** 이다. 코드가 말하는 '단' 은 실제로 '페이지' 이고,
    //     한 줄 13.7pt · 페이지 가용 47줄 → COLUMN_LINES 46 은 맞는 값이었다.
    //   - 캡쳐(폭 1054px 고정) 높이 ÷ 실제 줄 수 = **중앙값 53px/줄** (범위 46~70).
    //   - 상수별 과소평가(=넘칠 위험) 건수 / 총 줄수 (실제 390줄):
    //       30 → 0건 / 713줄 (1.8배 과대)   45 → 0건 / 482줄   48 → 1건   54 → 15건   67 → 넘침 발생
...
function sanitizeNodeStyles(node: any, validSets: {
    ...
    // → validStyles.CharShape.add('14')로 이미 보호됨. 여기서 다시 "14"로 강제 설정
    if (node.tagName === 'TEXT' || node.tagName === 'CHARSHAPE') {
        const hasDirectEndnote = Array.from(node.childNodes || []).some(
            (n: any) => n.nodeName === 'ENDNOTE' || n.nodeName === 'hp:ENDNOTE'
        );
        if (hasDirectEndnote) {
            node.setAttribute('CharShape', '14'); // Id="14" = Height=100, TextColor=16777215(흰색)
        } else if (forcedCs) {
