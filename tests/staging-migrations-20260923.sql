...
-- Requires the eight minimal public tables created on 2026-09-23.
-- Do not run on the live Supabase project.
BEGIN;
...
CREATE TABLE public.payment_orders (
 ...
 user_id uuid NOT NULL REFERENCES auth.users(id), kind text NOT NULL CHECK(kind IN ('cart','topup')),
 ...
 CHECK((kind='topup' AND points>0 AND used_points=0 AND jsonb_array_length(items)=0 AND amount=points+points/10)
 OR (kind='cart' AND points=0 AND jsonb_array_length(items) BETWEEN 1 AND 100))
);
...
BEGIN
 ...
 UPDATE public.profiles SET earned_points=coalesce(earned_points,0)-o.used_points,
   purchased_points=coalesce(purchased_points,0)+o.points
 ...
 IF o.points>0 OR o.used_points>0 THEN
   INSERT INTO public.point_transactions(user_id,type,amount,description,related_id)
   VALUES(p_user_id,CASE WHEN o.kind='topup' THEN 'charge' ELSE 'purchase' END,o.points-o.used_points,o.name,o.id);
 END IF;
...
COMMIT;
SELECT to_regclass('public.payment_orders') AS payment_orders, to_regclass('public.submission_earnings') AS submission_earnings, to_regclass('public.question_bank_events') AS question_bank_events, to_regprocedure('public.complete_payment_order(text,uuid)') AS complete_payment_order;
