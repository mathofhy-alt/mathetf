...
class MathPDFToHMLApp:
    ...
    def _setup_ui(self):
        ...
        # Variant Generation Checkbox
        self.generate_variants_var = tk.BooleanVar(value=False)
        self.variant_btn = tk.Checkbutton(main_frame, text="각 문항별 유사/변형 문제 3배수 자동 생성 (시간 더 소요됨)", variable=self.generate_variants_var)
        self.variant_btn.pack(anchor="w", pady=(0, 5))
        
        # Variant Difficulty Selection
        diff_frame = tk.Frame(main_frame)
        diff_frame.pack(fill="x", pady=(0, 15), padx=(20, 0)) # 들여쓰기 효과
        
        tk.Label(diff_frame, text="└ 변형 난이도:").pack(side="left")
        
        self.difficulties = [
            "1단계: 하 (단순 숫자/기호 변형 - 빠르고 안전함)",
            "2단계: 중 (조건/순서/구하는 값 변형 - 수능/모의고사 스타일)",
            "3단계: 상 (복합 개념 융합 및 실생활 응용 - 심화 고난도)"
        ]
        self.selected_difficulty = tk.StringVar(value=self.difficulties[0])
        self.diff_combo = ttk.Combobox(diff_frame, textvariable=self.selected_difficulty, values=self.difficulties, width=45, state="readonly")
    ...
    def _run_process(self):
        try:
            ...
            for idx, current_pdf in enumerate(self.pdf_paths, 1):
                ...
                problems = asyncio.run(parser.extract_math_problems(
                    current_pdf, 
                    log_callback=self._log, 
                    generate_variants=self.generate_variants_var.get(),
